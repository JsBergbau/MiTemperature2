...
def watchDog_Thread():
    ...
    while True:
        ...
        if (unconnectedTime is not None) and (
            ...
            try:
                bluepypid = re.findall(r"bluepy-helper\((.*)\)", pstree)[
                    0
            ...
            unconnectedTime = (
                now  # reset unconnectedTime to prevent multiple killings in a row
            )
        time.sleep(5)
...
def thread_SendingData():
    ...
    while True:
        try:
            ...
            if (
                mea == previousMeasurement and identicalCounter < args.skipidentical
            ...
            if (
                args.TwoPointCalibration or args.offset
    ...
    def handleNotification(self, cHandle, data):
        ...
        try:
            ...
            if args.round:
                ...
                if args.debounce:
                    ...
                    if fracpart >= 0.7:
                        ...
                        fracpart <= 0.2
                    ):  # either 0.8 and 0.3 or 0.7 and 0.2 for best even distribution
                        mode = "trunc"
                    # print("Modus: " + mode)
            ...
            if args.TwoPointCalibration:
                ...
                b = (
                    p2y - m * p2x
                )  # would be more efficient to do this calculations only once
                humidityCalibrated = m * humidity + b
                if (
                    humidityCalibrated > 100
                ):  # with correct calibration this should not happen
                    humidityCalibrated = 100
...
parser.add_argument(
    "--device",
    "-d",
    help="Set the device MAC-Address in format AA:BB:CC:DD:EE:FF",
    metavar="AA:BB:CC:DD:EE:FF",
)
parser.add_argument(
    "--battery",
    "-b",
    help="Get estimated battery level",
    metavar="",
    type=int,
    nargs="?",
    const=1,
...
parser.add_argument(
    "--count",
    "-c",
    help="Read/Receive N measurements and then exit script",
    metavar="N",
    type=int,
)
parser.add_argument(
    "--interface",
    "-i",
    help="Specifiy the interface number to use, e.g. 1 for hci1",
    metavar="N",
    type=int,
    default=0,
)
...
complexCalibrationGroup.add_argument(
    "--TwoPointCalibration",
    "-2p",
    help="Use complex calibration mode. All arguments below are required",
    action="store_true",
)
complexCalibrationGroup.add_argument(
...
complexCalibrationGroup.add_argument(
    "--offset1",
    "-o1",
    help="Enter the offset for the first calibration point",
    type=int,
)
complexCalibrationGroup.add_argument(
...
complexCalibrationGroup.add_argument(
    "--offset2",
    "-o2",
    help="Enter the offset for the second calibration point",
    type=int,
)
...
callbackgroup.add_argument(
    "--skipidentical",
    "-skip",
    help="N consecutive identical measurements won't be reported to callbackfunction",
    metavar="N",
    type=int,
    default=0,
)
...
if args.device:
    if re.match(
        "[0-9a-fA-F]{2}([:]?)[0-9a-fA-F]{2}(\\1[0-9a-fA-F]{2}){4}$", args.device
    ):
        adress = args.device
    else:
